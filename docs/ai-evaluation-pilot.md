...
Cycle assignments are derived on the server from Compass's physical evaluator mappings for the selected cohort, including individual relationships within the same department. DEPT, SELF, and existing 3E-excluded mappings stay outside this pilot. Mapping direction and relationship labels are preserved. HR reviews a read-only list rather than adding assignments manually. Missing employees and members without an eligible evaluator block activation. Draft saves refresh assignments; activation rejects mapping drift until HR saves and reviews again. Later mapping edits affect drafts and future cycles only. Existing legacy evaluation generation is unchanged by this mapping update.

### Initial benchmark and evolving profile

Each draft save builds the initial profile from the employee's Compass position and department, HR-entered responsibilities and expected outcomes, mapped relationship lenses, and the relationship-specific rubric and weights. A job title alone is not a sufficient responsibilities statement, so new members start with a blank expectations field for HR to complete. There is no invented starting performance score. The cycle configuration stores this baseline; HR reviews it before activation. Activation also rejects changed role metadata until the draft is saved again. The baseline, expectations, rubric, assignments, and weights stay frozen for that quarter.

Question generation, evidence synthesis, and assessment receive the frozen role and expectations. Department names and employee identifiers are not needed in inference requests. Versioned evidence profiles accumulate cited strengths, concerns, contradictions, and gaps separately from the initial benchmark. New evidence can change the assessment of performance, but cannot rewrite the standard. A later quarter requires a new reviewed benchmark. Older active pilot configurations without role metadata remain readable; new drafts acquire it on save.
