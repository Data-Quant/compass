...
Cycle assignments are now derived on the server from Compass's physical evaluator mappings for the selected cohort. Only mapped employees in different, nonempty departments participate; DEPT, SELF, same-department, and existing 3E-excluded mappings stay outside this pilot. Mapping direction and relationship labels are preserved. HR reviews a read-only list rather than adding assignments manually. Missing departments, inconsistent CROSS_DEPARTMENT mappings, and members without an eligible evaluator block activation. Draft saves refresh assignments; activation rejects mapping drift until HR saves and reviews again. The activation transaction freezes the assignments in the cycle configuration. Later mapping edits affect drafts and future cycles only; there is no silent mid-cycle reassignment. Existing legacy evaluation generation is unchanged by this mapping update.
